...
def transformar_d2(train_dataset):
    nsamples, nx, ny = train_dataset.shape
    X = train_dataset.reshape((nsamples,nx*ny))
    print(nsamples, nx, ny)
    print(nsamples, nx, ny)
    print("X d2: %s ", X.shape)
    logging.info("X d2: %s ", X.shape)
    return X
...
def train(X, y, path, filename, param_grid, columns):
    ...
    model = RANSACRegressor(random_state=0, min_samples=100)
    ...
    #results = model.fit(d2_train_dataset,y)
...
def main():    
    ...
    output="output/reg/"
    columns=["param_loss", "params", "mean_test_score", "std_test_score"]
    param_grid = {
